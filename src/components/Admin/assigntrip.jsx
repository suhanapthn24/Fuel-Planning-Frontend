...
export default function AssignTrip({ onBack }) {
  const yourAccessToken = localStorage.getItem("access_token");
  ...
  const handleSubmit = async (e) => {
    ...
    const tripId = `TRIP-${formData.depot_id}-${formData.truck_id}-${formData.driver_id}-${Date.now()}`;

    const payload = {
      delivery_schedule_id: tripId,
      ...
      route: [formData.origin, formData.destination], // or JSON.stringify if backend expects string
      delivered_liters: parseFloat(formData.delivered_liters),
      delivery_schedule_status: formData.trip_status,
      eta: parseFloat(formData.eta) / 60,
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/delivery_schedules", {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(error || "Failed to assign trip");
      }
  ...
  return (
    <div className="p-8 bg-white rounded-lg shadow-md w-full max-w-7xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Input label="Driver Username" name="driver_username" value={formData.driver_username} readOnly />
        <Input label="Driver Email" name="driver_email" value={formData.driver_email} readOnly />
        <Input label="Driver Name" name="driver_name" value={formData.driver_name} readOnly />
        ...
        <Input
          label="Delivered Liters"
          name="delivered_liters"
          type="number"
